...
beforeAll(async () => {
    ...
    await order_model_1.Order.deleteMany({ deliveryAddress: '123 Test Order Street' });
    await plant_model_1.Plant.deleteMany({ name: 'Order Test Fern' });
    await user_model_1.User.deleteMany({ email: { $regex: '@order-test\\.example$' } });
    // Seed plant
    const plant = await plant_model_1.Plant.create({
    ...
    const deliveryAdmin = await user_model_1.User.create({
        name: 'Delivery Admin',
        email: 'delivery@order-test.example',
        password: 'irrelevant-hashed',
        role: 'delivery_admin',
    });
    deliveryAdminToken = jsonwebtoken_1.default.sign({ id: deliveryAdmin._id.toString(), role: 'delivery_admin' }, process.env.JWT_SECRET, { expiresIn: '1h' });
    // Create order_admin
    const orderAdmin = await user_model_1.User.create({
        name: 'Order Admin',
        email: 'order-admin@order-test.example',
        password: 'irrelevant-hashed',
        role: 'order_admin',
    });
    orderAdminToken = jsonwebtoken_1.default.sign({ id: orderAdmin._id.toString(), role: 'order_admin' }, process.env.JWT_SECRET, { expiresIn: '1h' });
});
afterAll(async () => {
    await order_model_1.Order.deleteMany({ deliveryAddress: '123 Test Order Street' });
    await plant_model_1.Plant.deleteMany({ name: 'Order Test Fern' });
...
describe('POST /api/orders', () => {
    ...
    it('stock is decremented by ordered quantity', async () => {
        const plant = await plant_model_1.Plant.findById(plantId);
    ...
    it('policyAccepted missing → 400', async () => {
        ...
        const { policyAccepted: _omit, ...withoutPolicy } = payload;
        const res = await (0, supertest_1.default)(app_1.default)
            .post(BASE)
            .set('Authorization', `Bearer ${userToken}`)
            .send(withoutPolicy);
    ...
    it('unauthenticated → 401', async () => {
        const res = await (0, supertest_1.default)(app_1.default).post(BASE).send(validPayload());
...
describe('Soft delete behaviour', () => {
    it('order with isDeleted:true does not appear in GET /api/orders', async () => {
        ...
        await order_model_1.Order.findByIdAndUpdate(orderId, { isDeleted: true });
        const res = await (0, supertest_1.default)(app_1.default)
            .get(BASE)
            .set('Authorization', `Bearer ${userToken}`);
        expect(res.status).toBe(200);
        const returnedIds = res.body.data.map((o) => o._id);
