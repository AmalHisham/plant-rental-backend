...
beforeAll(async () => {
    ...
    await user_model_1.User.deleteMany({ email: { $regex: '@admin-test\\.example$' } });
    await order_model_1.Order.deleteMany({ deliveryAddress: '123 Admin Test Street' });
    await plant_model_1.Plant.deleteMany({ name: 'Admin Test Plant' });
    ...
    superAdminToken = jsonwebtoken_1.default.sign({ id: superAdmin._id.toString(), role: 'super_admin' }, process.env.JWT_SECRET, { expiresIn: '1h' });
    ...
    await order_model_1.Order.create({
