...
beforeAll(async () => {
    ...
    await user_model_1.User.deleteMany({ email: { $regex: '@cart-test\\.example$' } });
    await plant_model_1.Plant.deleteMany({ name: { $regex: /^Cart Test/ } });
    ...
    const lowStock = await plant_model_1.Plant.create({
    ...
    await cart_model_1.Cart.deleteMany({ userId: { $in: [user._id, otherUser._id] } });
});
afterAll(async () => {
    await user_model_1.User.deleteMany({ email: { $regex: '@cart-test\\.example$' } });
    await plant_model_1.Plant.deleteMany({ name: { $regex: /^Cart Test/ } });
...
it('12. Update item quantity → 200 with recalculated totals', async () => {
    const res = await (0, supertest_1.default)(app_1.default)
        ...
        .send({ quantity: 3 });
    expect(res.status).toBe(200);
    const updated = res.body.data.cart.items.find((i) => i.plantId._id === availablePlantId);
