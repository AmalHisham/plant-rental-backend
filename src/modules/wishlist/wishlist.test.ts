...
beforeAll(async () => {
  ...
  // Clean slate
  await User.deleteMany({ email: { $regex: '@wishlist-test\\.example$' } });
  await Plant.deleteMany({ name: { $regex: /^Wishlist Test/ } });
  ...
  await Wishlist.deleteMany({ userId: { $in: [user._id, otherUser._id] } });
});
